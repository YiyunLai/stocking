...
def candles(code):
    ...
    try:
        r=requests.get(BASE+f'/historical/candles/{code}',headers=HEAD,params={
            'from':(today-timedelta(days=190)).strftime('%Y-%m-%d'),
            'to':today.strftime('%Y-%m-%d'),'timeframe':'D',
            'fields':'open,high,low,close,volume','sort':'asc'},timeout=15)
...
def ret(cs,days):
    if not cs: return 0
    close=num(cs[-1]['close'])
    base=num(cs[0]['close']) if len(cs)<=days else num(cs[-days-1]['close'])
    return (close-base)/base*100 if base else 0

def pv(cs):
    if len(cs)<2: return {'kind':'資料不足','txt':'K線資料不足，暫時無法判讀價量。','v':0,'v5':0,'v20':0}
...
def sr(cs,close):
    ...
    p20=max(num(x['high']) for x in a); s20=min(num(x['low']) for x in a)
    p60=max(num(x['high']) for x in b); s60=min(num(x['low']) for x in b)
...
def svg(cs,W=860,PH=240,VH=80):
    ...
    cs=cs[-120:]; H=PH+VH+25
    hi=max(num(x['high']) for x in cs); lo=min(num(x['low']) for x in cs); vr=hi-lo or 1
    ...
    out.append('<text x="12" y="20" fill="#9ca3af" font-size="12">近120日價量走勢 黃線=MA20</text>')
    ...
    for k,c in enumerate(cs):
        x=k*cw+cw/2; o=num(c['open']); cl=num(c['close']); h=num(c['high']); l=num(c['low']); v=num(c['volume'])
        col='#ef4444' if cl>=o else '#22c55e'; yo=y(o); yc=y(cl)
...
def chip(s):
    f=num(s.get('外資今日(張)')); t=num(s.get('投信今日(張)')); fd=num(s.get('外資連買天數')); td=num(s.get('投信連買天數'))
    f5=num(s.get('外資5日累計(張)')); t5=num(s.get('投信5日累計(張)'))
    parts=[]
...
def tech(s):
    ...
    c=sum(1 for x in labels if x!='MA20翻揚' and s.get(x)=='是')
    txt='技術面多訊號共振，短線動能較強。' if c>=3 else ('技術面已有轉強跡象。' if c>=1 else '技術面尚未明顯轉強。')
    return tags,txt

def stock_page(s,cs,date):
    code=s.get('代號'); name=s.get('名稱'); close=num(s.get('收盤價')); chg=num(s.get('漲跌%'))
    pvv=pv(cs); srr=sr(cs,close); tags,tt=tech(s); color='#dc2626' if chg>=0 else '#16a34a'
...
def card(s):
    return f'<a class="card" href="stocks/{s.get("代號")}.html"><b>{s.get("代號")} {s.get("名稱")}</b><span>{s.get("籌碼類型")} · {s.get("類型")}</span><small>外資連{s.get("外資連買天數",0)}天｜投信連{s.get("投信連買天數",0)}天｜技術訊號{sum(1 for x in ["均線多頭","KD黃金交叉","MACD翻多","布林突破","爆量長紅"] if s.get(x)=="是")}</small></a>'
...
def make_pages(stocks,date):
    foreign=[('⭐ 雙主力連買','外資與投信同步連買。',[s for s in stocks if s.get('籌碼類型')=='雙主力連買']),('🔵 外資連買天數較高','依外資連買天數排序。',sorted([s for s in stocks if num(s.get('外資連買天數'))>0],key=lambda x:-num(x.get('外資連買天數')))[:40]),('💰 外資5日累計較大','依外資5日累計排序。',sorted([s for s in stocks if num(s.get('外資5日累計(張)'))>0],key=lambda x:-num(x.get('外資5日累計(張)')))[:40])]
    trust=[('⭐ 雙主力連買','外資與投信同步連買。',[s for s in stocks if s.get('籌碼類型')=='雙主力連買']),('🟣 投信連買天數較高','依投信連買天數排序。',sorted([s for s in stocks if num(s.get('投信連買天數'))>0],key=lambda x:-num(x.get('投信連買天數')))[:40]),('💰 投信5日累計較大','依投信5日累計排序。',sorted([s for s in stocks if num(s.get('投信5日累計(張)'))>0],key=lambda x:-num(x.get('投信5日累計(張)')))[:40])]
    techg=[('🔥 強勢噴出','分類為強勢噴出。',[s for s in stocks if s.get('類型')=='強勢噴出']),('📈 技術訊號共振','至少兩個技術訊號成立。',[s for s in stocks if sum(1 for x in ['均線多頭','KD黃金交叉','MACD翻多','布林突破','爆量長紅'] if s.get(x)=='是')>=2]),('🟡 低位啟動','初期轉強觀察名單。',[s for s in stocks if s.get('類型')=='低位啟動'])]
    return {'foreign.html':page('🔵 外資連買',foreign,date),'trust.html':page('🟣 投信連買',trust,date),'technical.html':page('🔥 技術面很強',techg,date)}

def index(date):
    return f'<!doctype html><html lang="zh-Hant"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>台股籌碼日報</title><style>body{{font-family:-apple-system,BlinkMacSystemFont,Segoe UI,sans-serif;background:#f5f5f5;color:#111827}}.wrap{{max-width:760px;margin:auto;padding:32px 16px}}.grid{{display:grid;grid-template-columns:repeat(auto-fit,minmax(230px,1fr));gap:14px}}a.card{{background:#fff;border:1px solid #e5e7eb;border-radius:16px;padding:18px;text-decoration:none;color:#111827}}</style></head><body><div class="wrap"><h1>📊 台股籌碼日報</h1><p>分類清單 → 個股價量走勢與籌碼分析 · 最新資料 {date}</p><div class="grid"><a class="card" href="foreign.html"><h2>🔵 外資連買</h2><p>外資連買、5日累計、雙主力</p></a><a class="card" href="trust.html"><h2>🟣 投信連買</h2><p>投信連買、5日累計、雙主力</p></a><a class="card" href="technical.html"><h2>🔥 技術面很強</h2><p>強勢噴出、技術共振、低位啟動</p></a><a class="card" href="holdings.html"><h2>📋 我的持股追蹤</h2><p>成本、現價、損益、法人籌碼</p></a></div></div></body></html>'
...
def main():
    ...
    pages=make_pages(stocks,date); pages['index.html']=index(date)
